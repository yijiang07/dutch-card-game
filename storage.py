...
def init_db():
    ...
    try:
        ...
        if USE_PG:
            for col in ('pw_salt', 'pw_hash', 'recovery_hash', 'email', 'lang', 'card_back', 'table_felt', 'emblem'):
...
def set_cosmetic(user_id, column, skin):
    ...
    assert column in ('card_back', 'table_felt', 'emblem')
    conn = _connect()
    try:
        cur = conn.cursor()
...
def get_by_id(uid):
    ...
    try:
        cur = conn.cursor()
        cur.execute(_ph('SELECT id, username, email, lang, card_back, table_felt, emblem FROM users WHERE id=?'), (uid,))
        r = cur.fetchone()
    finally:
        conn.close()
    return {'id': r['id'], 'username': r['username'], 'email': r['email'], 'lang': r['lang'],
            'card_back': r['card_back'] or 'classic', 'table_felt': r['table_felt'] or 'classic',
            'emblem': r['emblem'] or 'default'} if r else None
